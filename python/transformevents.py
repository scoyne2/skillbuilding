...
def main(outputTable, columns, collection):
    ...
    spark, sc, sqlContext = bc.init_spark("transformevents" + bc.DATABASE + "." + bc.CLEAN_TABLE)
    finalDf = bc.read_from_s3(spark, bc.CLEAN_TABLE).select(*columns).where(col("collection") == lit(collection)).distinct().withColumn(bc.PARTITION_COLUMN, current_date())  
...
if __name__ == "__main__":
    ...
    if args.collection == "project":
        arg_outputTable = bc.PROJECTS_EVENTS_TABLE
        ...
        arg_outputTable = bc.BID_PACKAGES_EVENTS_TABLE
        ...
        arg_outputTable = bc.BIDDER_GROUPS_EVENTS_TABLE

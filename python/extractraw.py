...
def main():
    ...
    try:
        rawDF = sqlContext.read.format("json").load(cfg.RAW_DATA_PATH).withColumn("ingest_date", current_date())
        #Rename and reorder columns
        df = rawDF.withColumnRenamed("@timestamp", "ingest_timestamp").withColumnRenamed("@uuid", "ingest_uuid").withColumnRenamed("timestamp", "event_timestamp").withColumnRenamed("type", "event_type")
        ...
        bc.write_table(df, cfg.RAW_TABLE)
